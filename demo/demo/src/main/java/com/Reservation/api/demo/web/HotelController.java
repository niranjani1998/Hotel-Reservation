...
public class HotelController {
  ...
  @RequestMapping(value = "/hotel", method = RequestMethod.PUT, produces = MediaType.APPLICATION_JSON_UTF8_VALUE)
    public ResponseEntity<Hotel> updateHotel(@RequestBody Hotel hotel) {
       ...
       return Optional.ofNullable(updateHotel)
               .map(b -> ResponseEntity.ok().body(b))
                .orElse(ResponseEntity.notFound().build());
   }
   ...
   @GetMapping(value = "/hotel/{hotelid}",produces = MediaType.APPLICATION_JSON_UTF8_VALUE)
    ...
    @CrossOrigin
    @GetMapping(value = "/hotel/{hotelid}/rooms",produces = MediaType.APPLICATION_JSON_UTF8_VALUE)
    public ResponseEntity<List<Room>> getRoomHotelById(@PathVariable Long hotelid){
        Optional<Hotel> optionalHotel = hotelService.getHotelById(hotelid);
        if (optionalHotel.isPresent()) {
            return new ResponseEntity<>(optionalHotel.get().getRooms(), HttpStatus.OK);
        }
        return new ResponseEntity<>(HttpStatus.NO_CONTENT);
    }
}
